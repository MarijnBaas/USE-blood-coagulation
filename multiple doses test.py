...
def pk_model(y, t, dose_i, dose_d, t_d, v_c, v_p, q, cl):
    c_c, c_p = y
    input_t = 0.0 # initialize input_t as zero
    if t == 0: # if it is the first time step, use the initial dose
        input_t = dose_i / v_c
    elif t % t_d == 0: # if it is a multiple of the dosing interval, use the additional dose
        update_dose(dose_d) # update the total dose
        input_t = dose_d / v_c
